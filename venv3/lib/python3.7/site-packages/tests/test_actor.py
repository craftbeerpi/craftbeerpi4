...
class ActorTestCase(AioHTTPTestCase):
    ...
    async def test_actor_mock(self):
        with mock.patch.object(
            self.cbpi.bus, "fire", wraps=self.cbpi.bus.fire
        ) as mock_obj:
            # Send HTTP POST
    ...
    async def test_actor_switch(self):
        ...
        resp = await self.client.post(
            path="/login", data={"username": "cbpi", "password": "123"}
        )
    ...
    async def test_crud(self):
        data = {"name": "CustomActor", "type": "CustomActor", "config": {"interval": 5}}
    ...
    async def test_actor_action(self):
        resp = await self.client.post(
            path="/actor/1/action",
            json=dict(name="myAction", parameter=dict(name="Manuel")),
        )
        assert resp.status == 204
